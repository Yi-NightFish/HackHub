...
class User(db.Model):
    ...
    def is_online(self):
        ...
        # Consider user online if last seen within the last 20 seconds
        return (now - self.last_seen) < datetime.timedelta(seconds=20)
...
class ChatVisibility(db.Model):
    ...
    visible_since = db.Column(db.DateTime, default=lambda: datetime.datetime(1970, 1, 1))
